...
let personName: string = "arham Afzal";
...
console.log(Name);
let withoutWitespace: string = Name.trim();
...
const myfavouriteNumber: number = 7;
...
console.log(
  "Unfortunately! new dinner table won't arrive in time for the dinner,so we can invite only two guests."
);
console.log();
...
//Original order
const PlacestoVisit: string[] = ["Paris", "Giza", "Kashmir", "Damascus"];
...
let greeting: string[] = ["Admin", "Arham", "Asad", "Ali ", "Waiz"];
...
let greetings: string[] = ["Admin", "Arham", "Asad", "Ali ", "Waiz"];
greetings = [];
...
let current_users: string[] = ["Arham", "Afzal", "User1", "User2", "Waiz"];
let new_users: string[] = ["John", "WAIZ", "user1", "Ali", "AfZAl"];
new_users.forEach((newOne) => {
  let condition = current_users.some(
    (current) => current.toLowerCase() === newOne.toLowerCase()
  );
  if (condition) {
...
let pizzas: string[] = ["pepperoni", "cheese", "chicken"];
...
function make_shirt(size: string, message: string) {
  console.log(
    `The size of the shirt is "${size}" and the text of the shirt is "${message}"`
  );
}
...
function makeShirt(
  size: string = "large",
  message: string = "Ilove typescript"
) {
...
let Magicians: string[] = ["Houdini", "Copperfield", "Blaine", "Angel"];
...
function make_great_copy(names: string[]): string[] {
...
function make_a_sandwich(...items: string[]): void {
